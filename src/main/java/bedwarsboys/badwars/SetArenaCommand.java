...
public class SetArenaCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (sender instanceof Player p) {
            ...
            if (args.length == 1 || args.length == 4) {
                if (args.length == 1 && (args[0].equals("1") | args[0].equals("2"))) {
                    ...
                    sender.sendMessage("Position " + args[0] + " saved successfully.");
                }
                Badwars.CONFIG.set("arena.pos" + pos, w + "," + x + "," + y + "," + z);
                Badwars.PLUGIN.saveConfig();
                sender.sendMessage("Position " + pos + " saved successfully.");
                return true;
            }
            if (args.length == 6) {
        ...
        sender.sendMessage("This command is currently for Players only.");
        return true;
    }
}
